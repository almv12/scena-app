...
export default function TeacherHome({ user }) {
  ...
  function handleCheckIn() {
    ...
    if (!navigator.geolocation) {
      ...
      setSubText('Ваш браузер не поддерживает геолокацию')
      return
    }
    ...
    navigator.geolocation.getCurrentPosition(
      async function(pos) {
        ...
        if (result.distance > maxDistance) {
          ...
          setSubText(result.branch.name + ' — ' + result.distance + 'м (макс ' + maxDistance + 'м)')
          setGpsError(true)
          setTimeout(function() {
            setCheckInStatus('Отметить приход')
            setSubText('Нажмите для check-in')
            setGpsError(null)
          }, 3000)
          return
        }
      ...
      function(err) {
        ...
        setSubText('Разрешите доступ к геолокации')
        setTimeout(function() {
          setCheckInStatus('Отметить приход')
          setSubText('Нажмите для check-in')
        }, 3000)
  ...
  return (
    <div className="page">
      ...
      <div className="card checkin-card">
        <button className={'checkin-btn' + (checkedIn ? ' checked' : '') + (gpsError ? ' error' : '')} onClick={handleCheckIn}>
          {checkedIn ? '✓' : '📍'}
        </button>
        <div className="checkin-label">{checkInStatus}</div>
        <div className="checkin-sub">{subText}</div>
        {checkedIn && (
          <button className="btn btn-secondary" style={{ marginTop: 12, width: 'auto', display: 'inline-block', padding: '8px 24px' }} onClick={handleCheckOut}>
            Отметить уход
          </button>
        )}
      </div>
